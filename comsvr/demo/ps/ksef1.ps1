...
try {
    ...
    $ksef.FormCode = 1 # Wersja struktury wysylanych plikow FA: 1 - FA(1), 2 - FA(2). 0 - wlasny, wymaga wypelnienia FormCodeSystemCode, FormCodeSchemaVersion, FormCodeTargetNamespace, FormCodeValue
    ...
    do {
        $response = $ksef.SessionStatus('', 10, 0, $false)
        Write-Host 'Status sesji: ' $response.ProcessingCode()
        Write-Host 'Opis: ' $response.ProcessingDescription()
        Write-Host 'Dowolny klawisz aby próbować ponownie'
        $null = $Host.UI.RawUI.ReadKey('NoEcho,IncludeKeyDown')
    }
    until ($response.ProcessingCode() -ge 315)

    if ($response.ProcessingCode() -lt 400) {
