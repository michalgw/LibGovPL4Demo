...
uses
  ...
  Dialogs, ComCtrls, StdCtrls, ExtCtrls, CheckLst, Menus, Contnrs, Spin, Grids,
  LibGovPL4Intf, LibGovPL4Backend, LibGovPL4XAdES, LibGovPL4KSeF2, LibGovPL4KSeFObj2;
...
uses
  uFormCertLoad, TypInfo, IniFiles, Unit2, LibGovPl4Obj;
...
begin
  ...
  for I := 0 to CheckListBoxSigners.Count - 1 do
    ...
    begin
      if SameText(CheckListBoxSigners.Items[I], 'TlgPKCS11CertificateSigner') then
      {try
        Signer := TlgoPKCS11CertificateSigner.Create(CheckListBoxSigners.Items[I]);
        ...
        Signer := TlgoCertificateSigner.Create(CheckListBoxSigners.Items[I]);
  ...
  for I := 0 to lgplDriverCount(LGP_CLSTYPE_RSA_ENC) - 1 do
  begin
    MI := TMenuItem.Create(PopupMenuKeys);
    MI.Caption := lgplDriverName(LGP_CLSTYPE_RSA_ENC, I);
...
procedure TForm1.CertUISelect(ASigner: TlgoCertificateSigner);
var
...
procedure TForm1.CertLoadFromFile(ASigner: TlgoCertificateSigner);
var
  ...
  begin
    try
      ...
      if not Assigned(Certificates) then
        Certificates := TObjectList.Create(True);
...
function CertificateKeyUsageToStr(AValue: TlgoCertificateKeyUsage): String;
const
  STRS: array[TlgoCertificateKeyUsageType] of String = ('ckuDigitalSignature',
    'ckuNonRepudiation', 'ckuKeyEncipherment', 'ckuDataEncipherment',
    'ckuKeyAgreement', 'ckuKeyCertSign', 'ckuCRLSign', 'ckuEncipherOnly',
    'ckuDecipherOnly');
var
  I: TlgoCertificateKeyUsageType;
begin
  Result := '';
  for I := Low(TlgoCertificateKeyUsageType) to High(TlgoCertificateKeyUsageType) do
    if I in AValue then
    begin
      if Result <> '' then
        Result := Result + ',';
...
begin
  ...
  MemoLog.Lines.Add('  U¿ycie: ' + CertificateKeyUsageToStr(ACertyfikat.KeyUsage));
...
begin
  ...
  begin
    Certs := TlgoCertificateSigner(Signers[I]).List;
...
begin
  ...
  for I := 0 to Certificates.Count - 1 do
    with TlgoCertificate(Certificates[I]) do
    begin
      ...
      LI.SubItems.Add(CertificateKeyUsageToStr(KeyUsage));
      ...
      DebugCert(TlgoCertificate(Certificates[I]));
    end;
end;
  ...
  if AException is EKSeF2ExceptionResponse then
    ...
    begin
      ...
      begin
        ...
        Debug('    Details: ' + ExceptionDetailList[I].ExceptionDetails);
        Debug('  ]');
      end;
    end;
  ...
  if AException is EKSeF2BadRequest then
    ...
    begin
      ...
      begin
        ...
        Debug('    Details: ' + Errors[I].ExceptionDetails);
        Debug('  ]');
      end;
    end;
...
begin
  ...
  I := FS.Size;
  FS.Free;
  PackageCount := (I + Pred(PartSize)) div PartSize;
...
var
  FileStream: TFileStream;
  KomunikatWeryfikacji: UTF8String;
begin
  FileStream := nil;
  Debug('Pobranie UPO faktury z sesji na podstawie numeru KSeF', True);
  if not TlgoKSeF2Utils.IsKsefNumberValid(EditKSeFUKSefNumber.Text, KomunikatWeryfikacji) then
  begin
...
begin
  ...
  if not TlgoKSeF2Utils.IsKsefNumberValid(EditKSeFDKsefNumber.Text, KomunikatWeryfikacji) then
  begin
    Debug('Nr Ksef faktury jest nieprawid³owy: ' + KomunikatWeryfikacji);
    Exit;
  end;
...
begin
  ...
  try
    // Zwolnij poprzedni¹ odpowiedŸ
    if Assigned(ExportResponse) and (ListViewObj.FindData(0, Pointer(ExportResponse), True, True) = nil) then
      ExportResponse.Free;
    ExportResponse := nil;
    // Wyczyœæ nazwê pliku docelowego i zablokuj pola pobierania
    FileNameEditKSeFDEFileName.Text := '';
    GroupBoxKSeFD1.Enabled := False;
    // Generuj filtr na podstawie którego zostanie przygotowana paczka faktur
    Filter := GenerateFilter;
    AddObject(Filter);
    // Za¿¹daj przygotowania paczki faktur, zapisz nr referencyjny procesu do pola edycji.
...
begin
  ...
  try
    ...
    // Status = 200 czyli zakoñczono proces tworzenia paczki na serwerze
    if (Response.Status.Code = 200) then
    begin
      // Czy odpowiedŸ zawiera jakiekolwiek czêœci paczki?
      // Jeœli tak to zezwól na pobranie
      if (Response.Package.Parts.Count > 0) then
      begin
        Debug('Iloœæ czêœci: ' + IntToStr(Response.Package.Parts.Count));
        if Assigned(ExportResponse) and (ListViewObj.FindData(0, Pointer(ExportResponse), True, True) = nil) then
        begin
          ExportResponse.Free;
          ExportResponse := nil;
        end;
        // Zachowaj odpowiedŸ zapytania o status - zawiera informacje o czêœciach paczki.
        // Bêdzie potrzebna przy pobieraniu czêœci.
        ExportResponse := Response;
        // Odblokuj mo¿liwoœæ pobrania paczki.
        GroupBoxKSeFD1.Enabled := True;
        // Ustaw domyœln¹ nazwê pliku paczki.
        FileNameEditKSeFDEFileName.Text := EditKSeFDEOperationReferenceNumber.Text +
...
begin
  ...
  try
    try
      ...
      // Pobieramy paczkê. Pobrane i odzyfrowane zostan¹ wszystkie czêœci.
...
const
 BC = '-----BEGIN CERTIFICATE-----'#10;
 BE = '-----END CERTIFICATE-----';
begin
  BStream := TMemoryStream.Create;
  BStream.Write(BC[1], Length(BC));
  BStream.Write(AKeyData[1], Length(AKeyData));
  BStream.Write(BE[1], Length(BE));
  BStream.Position := 0;
  Result :=  TlgoRSAPublicKey.CreateKey(KSeF.RSAPublicKeyClass, BStream, letPEM);
  FreeAndNil(BStream);
end;
...
begin
  EditKSeFLLink.Text := TlgoKSeF2VerificationLinkService.BuildInvoiceVerificationUrl(
...
begin
  if ComboBoxLCertificate2.ItemIndex >= 0 then
    EditKSeFLLink1.Text := TlgoKSeF2VerificationLinkService.BuildCertificateVerificationUrl(
...
begin
  ...
  try
    ...
    Metadata := TlgoKSeF2Utils.LoadInvoiceMetadataFromStream(FileStream);
    AddObject(Metadata);
...
begin
  Debug('Pobranie listy faktur wchodz¹cych w sk³ad identyfikatora zbiorczego', True);
  if EditColIdIId.Text = '' then
  begin
    MessageDlg('WprowadŸ identyfikator', mtInformation, [mbOK], 0);
    Exit;
  end;
  try
